...
(function(){
  const items = [
    ...
    'EFFORT GRAND <b>$50,000</b>','MOST TIMEBACK XP WINS','<b>FREE</b> ALL SEASON','90 ZIP TEAMS','<b>$1,000</b> WINNERS CITYWIDE','19-WEEK SEASON','WINTER GRIND <b>DEC 19–JAN 3</b>',
...
function lookupZip(zip){
  ...
  if(!rec){
    ...
    el.innerHTML = `<div class="zr-top"><div class="zr-zip">${/^\d{5}$/.test(zip)?zip:'-'}</div></div>
      <p style="color:var(--dim)">That zip isn't in the 5-county Austin metro (90 residential zips: Travis, Williamson, Hays, Bastrop, Caldwell). Try one of the samples above.</p>`;
    return;
  }
  const kids = rec.kids;
